...
const MenuSection = () => {
  ...
  const productData = [
    {
      category: { az: "Dezinfeksiya Məhsulları" },
      items: [
        {
          id: "1",
    ...
    {
      category: { az: "Təmizləyici Məhsullar" },
      items: [
        {
          id: "5",
  ...
  const processMenuData = (data) => {
    ...
    return formattedData.reduce((acc, cat) => {
      if (!cat || !cat.category || !cat.category.az) return acc;
  ...
  useEffect(() => {
    try {
      setMenuItems(processMenuData(productData));
      setFetchError(null);
    } catch (error) {
  ...
  useEffect(() => {
    ...
    if (searchTerm.trim() !== "") {
      ...
      items = items.filter((item) => {
        const nameMatch = (item.name?.az || "").toLowerCase().includes(term);
        const descMatch = (item.description?.az || "").toLowerCase().includes(term);
                  ...
                  <div className="menu-filter-item">
                    <motion.button
                      className={`menu-filter-button ${
                        activeFilter === "all" ? "menu-filter-button-active" : ""
                      }`}
                  ...
                  {categories.map((category) => (
                    <div key={category} className="menu-filter-item">
                      <motion.button
                        ...
                        whileTap={{ scale: 0.95 }}
                      >
                        {menuItems[category]?.name?.az || category}
                      </motion.button>
                    </div>
                  ))}
                </div>
